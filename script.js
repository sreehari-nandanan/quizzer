...
let usedQuestionIndices = new Set(); // Track used question indices
...
function getRandomQuestionIndex(totalQuestions) {
  if (usedQuestionIndices.size >= totalQuestions) {
      // If we've used all questions, reset (though this shouldn't happen with 10 questions)
...
function startQuiz() {
  ...
  // Reset used questions for new quiz
  usedQuestionIndices.clear();
  ...
  loadJSON(path)
      .then(data => {
          ...
          for (let i = 0; i < Math.min(10, questions.length); i++) {
              const randomIndex = getRandomQuestionIndex(questions.length);
...
function nextQuestion() {
  ...
  if (selectedOption === correctIndex) {
      ...
      score = Math.min(score + 10, selectedQuestions.length * 10); // Ensure score doesn't exceed max
  } else {
...
function showResults() {
  ...
  const percent = Math.min(Math.round((score / total) * 100), 100); // Ensure percent doesn't exceed 100
